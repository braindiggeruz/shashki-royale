...
export default function OnlineGame() {
  ...
  return (
    <div
      ...
      style={{ background: "radial-gradient(ellipse at center, #2C1810 0%, #0A0503 100%)" }}
    >
      {/* Header — compact: back / status / flag */}
      <div
      ...
      >
        ...
        <div className="flex items-center gap-2" data-testid="online-status">
          <div
      ...
      {/* Compact turn indicator + inline warnings */}
      <div className="px-3 pt-2 flex-shrink-0">
        <motion.div
          key={`${gameState.currentTurn}-${isMyTurn}-${hasMandatory}`}
        ...
        <AnimatePresence>
          {showOpponentMoved && (
            <motion.div
              ...
              className="flex items-center justify-center gap-1.5 mt-1 py-1 px-2 rounded-md"
              style={{ background: "rgba(212,175,55,0.10)", border: "1px solid rgba(212,175,55,0.22)" }}
            >
              <CheckCircle2 className="w-3 h-3 flex-shrink-0" style={{ color: "#D4AF37" }} />
              <span className="text-[11px]" style={{ color: "#FFD700" }}>Соперник походил</span>
            </motion.div>
          )}
          {connectionLost && (
            <motion.div
              ...
              exit={{ opacity: 0, height: 0 }}
              className="flex items-center justify-center gap-1.5 mt-1 py-1 px-2 rounded-md"
              style={{ background: "rgba(150, 100, 0, 0.20)", border: "1px solid rgba(200,150,0,0.30)" }}
            >
              <WifiOff className="w-3 h-3 text-yellow-400 flex-shrink-0" />
              <span className="text-[11px] text-yellow-300">Переподключение...</span>
            </motion.div>
          )}
          {syncError && (
            <motion.div
              ...
              exit={{ opacity: 0, height: 0 }}
              className="flex items-center justify-center gap-1.5 mt-1 py-1 px-2 rounded-md"
              style={{ background: "rgba(200, 50, 0, 0.20)", border: "1px solid rgba(220,80,0,0.35)" }}
            >
      ...
      {/* Opponent Player Card (top) — with their captured count */}
      <div className="px-3 pt-2 flex-shrink-0">
        <PlayerCard
          ...
          capturedCount={
            ...
            gameState.board
              .flat()
              .filter((c) => c?.color === (myColor === "white" ? "black" : "white"))
              .length
          }
        />
      </div>
      ...
      {/* My Player Card (bottom) — with my captured count */}
      <div className="px-3 pb-3 pt-1 flex-shrink-0">
        <PlayerCard
          ...
          capturedCount={
            12 -
            gameState.board.flat().filter((c) => c?.color === myColor).length
          }
          isMe
        />
      </div>
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-center mb-4" style={{ color: "rgba(200,160,80,0.7)" }}>
                Текущая партия будет засчитана как поражение.
              </p>
